  ...
  return (
    <main className="login-shell">
      <section className="login-card" aria-labelledby="login-title">
        ...
        <div className="login-footer"><button className="text-button" type="button" onClick={() => void resetPassword()} disabled={busy}>He olvidado mi contraseña</button></div>
      </section>
    </main>
  );
}
...
export function App() {
  ...
  const selectedHolding = useMemo(() => holdings.find((item) => item.id === selectedHoldingId) ?? null, [holdings, selectedHoldingId]);
  const selectedFarm = useMemo(() => farms.find((item) => item.id === selectedFarmId) ?? null, [farms, selectedFarmId]);
  ...
  const loadCampaign = useCallback(async (campaignId: string) => {
    ...
    const [deliveryResult, campaignSummary] = await Promise.all([api.deliveries(campaignId), api.campaignSummary(campaignId)]);
  ...
  useEffect(() => { void loadHoldingData(selectedHoldingId).catch((reason) => setError(messageFrom(reason))); }, [selectedHoldingId, loadHoldingData]);
  ...
  async function runAction(action: () => Promise<void>) {
    setBusy(true);
    setError(null);
    try { await action(); } catch (reason) { setError(messageFrom(reason)); } finally { setBusy(false); }
  }
  ...
  if (sessionState === 'signed_out' || !user) return <LoginScreen onSignedIn={(signedInUser) => { setUser(signedInUser); setSessionState('signed_in'); }} />;
  ...
  return (
    <div className="app-shell">
      <header className="app-header">
        <div className="brand-lockup" aria-label="Mágina Olivo"><span className="brand-title">Mágina Olivo</span><span className="brand-kicker">Sierra Mágina · Jaén</span></div>
        <button type="button" className="user-chip" onClick={() => setTab('more')} aria-label="Abrir perfil">{initials}</button>
      </header>
      <main className="page">
        {error ? <div className="alert" role="alert">{error}</div> : null}
        {holdings.length > 1 ? <select className="selector" value={selectedHoldingId} onChange={(event) => setSelectedHoldingId(event.target.value)} aria-label="Explotación activa">{holdings.map((holding) => <option key={holding.id} value={holding.id}>{holding.name}</option>)}</select> : null}
...
function HomeTab({ holding, campaign, summary, coverage, onNavigate }: { holding: Holding | null; campaign: Campaign | null; summary: CampaignSummary | null; coverage: number; onNavigate: (tab: Tab) => void }) {
  return <>
    ...
    <section className="section"><div className="section-heading"><div><h2 className="section-title">Hoy en tu olivar</h2><p className="section-copy">Prioridades reales de campaña, sin ruido.</p></div></div><article className="card list-card"><div className="list-card-main"><p className="list-card-title">Rendimientos pendientes</p><p className="list-card-meta">Añade el resultado cuando te lo facilite la almazara.</p></div><span className="badge gold">{summary?.pendingResultCount ?? 0}</span></article></section>
  </>;
}
...
function FieldTab({ holdings, selectedHolding, farms, selectedFarm, selectedFarmId, plots, busy, setSelectedFarmId, runAction, reloadHoldings, reloadHoldingData, reloadPlots }: { holdings: Holding[]; selectedHolding: Holding | null; farms: Farm[]; selectedFarm: Farm | null; selectedFarmId: string; plots: Plot[]; busy: boolean; setSelectedFarmId: (id: string) => void; runAction: ActionRunner; reloadHoldings: () => Promise<void>; reloadHoldingData: () => Promise<void>; reloadPlots: () => Promise<void> }) {
  return <>
    ...
    {selectedHolding ? <>
      <section className="section"><div className="section-heading"><div><h2 className="section-title">Fincas</h2><p className="section-copy">{selectedHolding.municipality || 'Sierra Mágina'}{selectedHolding.province ? ` · ${selectedHolding.province}` : ''}</p></div></div>
        {farms.map((farm) => <button key={farm.id} type="button" className="card list-card interactive" style={{ width: '100%', textAlign: 'left' }} onClick={() => setSelectedFarmId(farm.id)}><div className="list-card-main"><p className="list-card-title">{farm.name}</p><p className="list-card-meta">{farm.areaHa ? `${farm.areaHa} ha` : 'Superficie pendiente'}</p></div><span className={`badge${farm.id === selectedFarmId ? ' gold' : ''}`}>{farm.id === selectedFarmId ? 'Activa' : 'Ver'}</span></button>)}
...
function CampaignTab({ selectedHolding, campaigns, selectedCampaignId, setSelectedCampaignId, selectedCampaign, farms, plots, deliveries, summary, busy, runAction, reloadHoldingData, reloadCampaign }: { selectedHolding: Holding | null; campaigns: Campaign[]; selectedCampaignId: string; setSelectedCampaignId: (id: string) => void; selectedCampaign: Campaign | null; farms: Farm[]; plots: Plot[]; deliveries: Delivery[]; summary: CampaignSummary | null; busy: boolean; runAction: ActionRunner; reloadHoldingData: () => Promise<void>; reloadCampaign: () => Promise<void> }) {
  return <>
    ...
    {campaigns.length ? <section className="section"><select className="selector" value={selectedCampaignId} onChange={(event) => setSelectedCampaignId(event.target.value)} aria-label="Campaña activa">{campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select></section> : null}
    {selectedCampaign ? <>
      <section className="section card card-body"><div className="metrics" style={{ marginTop: 0 }}><Metric value={formatKg(summary?.totalKilograms)} label="kilos" /><Metric value={formatPercent(summary?.weightedYieldPercent)} label="rendimiento" /></div></section>
      ...
      <section className="section"><div className="section-heading"><div><h2 className="section-title">Entregas</h2><p className="section-copy">{deliveries.length} registradas en esta campaña.</p></div></div>
        {deliveries.map((delivery) => <article className="card delivery-row" key={delivery.id}><div><div className="delivery-kilos">{formatKg(delivery.kilograms)}</div><div className="delivery-date">{new Date(delivery.deliveredAt).toLocaleDateString('es-ES')} · {delivery.customDestination || 'Cooperativa'}{delivery.ticketNumber ? ` · Ticket ${delivery.ticketNumber}` : ''}</div></div><YieldForm deliveryId={delivery.id} busy={busy} runAction={runAction} onCreated={reloadCampaign} /></article>)}
...
function CreateCampaignCard({ holdingId, busy, runAction, onCreated }: { holdingId: string; busy: boolean; runAction: ActionRunner; onCreated: () => Promise<void> }) {
  ...
  return <FormCard title="Crear campaña" submitLabel="Abrir campaña" busy={busy} onSubmit={(form) => runAction(async () => { const seasonStartYear = Number(form.get('seasonStartYear') || year); await api.createCampaign(holdingId, { name: String(form.get('name') || '').trim(), seasonStartYear }); await onCreated(); })} fields={<div className="inline-fields"><Field name="name" label="Nombre" placeholder={`Campaña ${year}/${String(year + 1).slice(-2)}`} required /><Field name="seasonStartYear" label="Año inicio" type="number" defaultValue={String(year)} required /></div>} />;
}

function CreateDeliveryCard({ campaignId, farms, plots, busy, runAction, onCreated }: { campaignId: string; farms: Farm[]; plots: Plot[]; busy: boolean; runAction: ActionRunner; onCreated: () => Promise<void> }) {
  return <FormCard title="Nueva entrega" submitLabel="Guardar entrega" busy={busy} onSubmit={(form) => runAction(async () => {
    ...
    const body: { deliveredAt: string; kilograms: string; customDestination: string; farmId?: string; plotId?: string; ticketNumber?: string; clientGeneratedId: string } = {
      deliveredAt: new Date(String(form.get('deliveredAt'))).toISOString(),
      kilograms: String(form.get('kilograms') || '').trim(),
      customDestination: String(form.get('destination') || '').trim(),
      clientGeneratedId,
    };
    if (farmId) body.farmId = farmId;
    if (plotId) body.plotId = plotId;
    if (ticketNumber) body.ticketNumber = ticketNumber;
    await api.createDelivery(campaignId, body, clientGeneratedId);
    await onCreated();
...
function YieldForm({ deliveryId, busy, runAction, onCreated }: { deliveryId: string; busy: boolean; runAction: ActionRunner; onCreated: () => Promise<void> }) {
  ...
  return <form className="yield-form" aria-label="Añadir rendimiento" onSubmit={(event) => { event.preventDefault(); if (!value) return; void runAction(async () => { await api.createYield(deliveryId, value); setValue(''); await onCreated(); }); }}><label className="sr-only" htmlFor={`yield-${deliveryId}`}>Rendimiento porcentual</label><input id={`yield-${deliveryId}`} type="number" min="0" max="100" step="0.01" placeholder="21,7" value={value} onChange={(event) => setValue(event.target.value)} /><button type="submit" disabled={busy || !value}>% +</button></form>;
}
...
function FormCard({ title, submitLabel, busy, fields, onSubmit }: { title: string; submitLabel: string; busy: boolean; fields: ReactNode; onSubmit: (form: FormData) => Promise<void> }) {
  return <section className="section card card-body"><h2 className="section-title form-card-title">{title}</h2><form className="form-grid" onSubmit={(event) => { event.preventDefault(); const target = event.currentTarget; const form = new FormData(target); void onSubmit(form).then(() => target.reset()); }}>{fields}<div className="form-actions"><button className="primary-button" type="submit" disabled={busy}>{busy ? 'Guardando…' : submitLabel}</button></div></form></section>;
}
