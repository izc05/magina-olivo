...
import { Gift, PackageCheck, Sprout } from 'lucide-react';
...
export function RewardCatalogPage() {
  ...
  return (
    <div className="rewards-shell">
      <VisualHeader />
      <main className="rewards-page" id="main-content">

      <section className="rewards-hero">
        <span className="rewards-kicker">Mi olivo</span>
        <h1>Recompensas</h1>
        <p>Canjea tus aceitunas por premios y experiencias con stock confirmado.</p>
        <div className="rewards-balance" aria-label={`${availableOlives} aceitunas disponibles`}><Sprout aria-hidden="true" /><span>Saldo actual</span><b>{formatOlives(availableOlives)}</b><small>aceitunas disponibles</small></div>
      </section>
      ...
      <section className="rewards-section">
        ...
        {catalog.length === 0 ? (
          <article className="rewards-coming-card">
            <span className="rewards-product-icon" aria-hidden="true"><Gift /></span>
            <div>
              ...
              <strong>Objetivo de referencia: 2.500 aceitunas por botella de 500 ml</strong>
            </div>
          </article>
        ) : (
              ...
              return (
                <article className="reward-card" key={reward.id}>
                  <div className="reward-card-top">
                    <span className="rewards-product-icon" aria-hidden="true"><PackageCheck /></span>
                    <div>
                  ...
                  <div className="reward-metrics">
                    <span><b>{formatOlives(reward.costOlives)} aceitunas</b><small>Coste</small></span>
                  ...
                  <button type="button" disabled={disabled} onClick={() => void redeem(reward)}>
                    {workingRewardId === reward.id
                      ...
                      : !hasStock
                        ? 'Agotado'
                        : !canAfford
                          ? `Te faltan ${formatOlives(reward.costOlives - availableOlives)} aceitunas`
                          : 'Canjear recompensa'}
                  </button>
                </article>
              );
