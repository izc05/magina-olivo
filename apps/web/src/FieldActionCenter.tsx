  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (selectedAction.form === 'activity') {
        await api.createActivity(holdingId, { activityType: (values.get('activityType') as ActivityType) || selectedAction.activityType || 'observation', occurredAt: isoDateTime(values.get('occurredAt')), ...(campaignId ? { campaignId } : {}), ...(farmId ? { farmId } : {}), ...(plotId ? { plotId } : {}), ...(area !== null ? { affectedAreaHa: area } : {}), ...(String(values.get('product') || '') ? { productName: String(values.get('product')) } : {}), ...(quantity !== null ? { quantity } : {}), ...(String(values.get('unit') || '') ? { quantityUnit: String(values.get('unit')) } : {}), ...(cost !== null ? { costEur: cost } : {}), ...(String(values.get('notes') || '') ? { notes: String(values.get('notes')) } : {}) });
