...
export function PublicHomePage() {
  ...
  return (
    <main className="public-home public-home-v2" id="main-content">
      <VisualHeader />
      <section className={`public-home-v2-hero ${weatherMood}`} aria-label="Nuestra tierra"><div className="public-home-v2-photo" role="img" aria-label="Olivar de Sierra Mágina" /><a href="/magina/tiempo" className={`public-home-weather-card ${weatherMood}`} aria-live="polite"><span className="weather-orb" aria-hidden="true"><i /></span><span>{weatherTitle}</span><strong>{weatherTemperature}</strong><small>{weatherRange}</small>{weather ? <small>Previsión AEMET · Ver detalle <ChevronRight size={14} aria-hidden="true" /></small> : <small>Consultar previsión <ChevronRight size={14} aria-hidden="true" /></small>}</a><div className="home-territory-caption"><p>Nuestra tierra,<br />tu mejor cosecha</p><small>Sierra Mágina</small></div></section>
