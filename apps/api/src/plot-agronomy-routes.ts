...
type PlotAccessRow = { farm_id: string; holding_id: string };
type AgronomyRow = {
  id: string;
  name: string;
...
export function registerPlotAgronomyRoutes(app: FastifyInstance): void {
  app.get<{ Params: FarmParams }>('/api/v1/farms/:farmId/plots/agronomy', async (request, reply) => {
    ...
    if (!access) return reply.code(404).send(apiError(request, 'FARM_NOT_FOUND', 'Farm not found'));
    const result = await getPool().query<AgronomyRow>(
      `select id, name, olive_tree_count, irrigation_type, olive_variety, area_ha, boundary_area_ha
       from plots where farm_id = $1 and holding_id = $2 and active = true
  ...
  app.patch<{ Params: PlotParams; Body: UpdateAgronomyBody }>(
    '/api/v1/plots/:plotId/agronomy',
    {
      schema: {
        body: {
          ...
          properties: {
            oliveTreeCount: { anyOf: [{ type: 'integer', minimum: 0, maximum: 100000000 }, { type: 'null' }] },
            irrigationType: { anyOf: [{ type: 'string', enum: ['dryland', 'irrigated', 'mixed', 'unknown'] }, { type: 'null' }] },
            oliveVariety: { anyOf: [{ type: 'string', minLength: 1, maxLength: 80 }, { type: 'null' }] },
          },
        },
      },
    },
    async (request, reply) => {
      const session = await getAuthenticatedSession(request);
      if (!session) return reply.code(401).send(apiError(request, 'AUTH_REQUIRED', 'Authentication required'));
      const plotLookup = await getPool().query<PlotAccessRow>('select farm_id, holding_id from plots where id = $1 and active = true', [request.params.plotId]);
      const plot = plotLookup.rows[0];
      if (!plot) return reply.code(404).send(apiError(request, 'PLOT_NOT_FOUND', 'Plot not found'));
      const access = await getFarmAccess(session.user.id, plot.farm_id);
      if (!access || access.holdingId !== plot.holding_id) return reply.code(404).send(apiError(request, 'PLOT_NOT_FOUND', 'Plot not found'));
      ...
      const updated = await getPool().query<AgronomyRow>(
        `update plots set olive_tree_count = $1, irrigation_type = $2, olive_variety = $3,
        ...
        [request.body.oliveTreeCount, request.body.irrigationType, variety, request.params.plotId, access.holdingId],
      );
      const row = updated.rows[0];
      if (!row) return reply.code(404).send(apiError(request, 'PLOT_NOT_FOUND', 'Plot not found'));
