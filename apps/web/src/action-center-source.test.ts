...
test('the global action center keeps additions in one contextual flow', async () => {
  ...
  for (const label of ['Trabajo', 'Tratamiento', 'Riego', 'Recolección', 'Entrega', 'Tarea', 'Nueva parcela', 'Nueva finca']) assert.ok(center.includes(label), label);
  assert.match(center, /productRegistrationNumber/);
  assert.match(center, /Kilos estimados/);
  assert.match(center, /Registrar/);
  assert.match(center, /Organizar/);
  assert.match(center, /field-action-groups/);
  ...
  assert.match(app, /tab === 'field' \|\| tab === 'campaign' \? <button type="button" className=\{`nav-plus/);
