...
function WeatherRadarPreview() {
  ...
  if (!frame) return <section className="card weather-radar-mini"><div className="weather-radar-mini-heading"><div><p className="eyebrow">RADAR DE LLUVIA</p><strong>Precipitación sobre Sierra Mágina</strong></div><a className="text-button" href="/magina/tiempo/radar">Abrir radar <ChevronRight aria-hidden="true" /></a></div><div className="weather-radar-mini-viewport"><img src="/photos/sierra-magina-radar-visual.png" alt="Vista visual de precipitación sobre Sierra Mágina" /><span>Vista demo · esperando fotogramas AEMET</span></div></section>;
...
export function WeatherHourlyPage() {
  ...
  const selectedDay = selectedAt ? new Intl.DateTimeFormat('es-ES', { weekday: 'long', day: 'numeric', month: 'long' }).format(selectedAt) : 'Consultando fecha';
  return <WeatherShell><WeatherIntro title="Previsión por horas" copy={`Consulta la evolución del tiempo en ${data?.municipality.name ?? 'tu municipio'} y planifica tus labores en Sierra Mágina.`} />
    ...
    <section className="card weather-hour-summary"><div><p className="eyebrow">HOY</p><small>{selectedDay}</small><WeatherIcon rain={selected?.precipitationProbabilityPercent} night={selectedAt ? selectedAt.getHours() >= 21 || selectedAt.getHours() < 7 : false} /><strong>{fmt(selected?.temperatureC ?? null, '°')}</strong></div><dl><div><dt><Droplets />Humedad media</dt><dd>{fmt(selected?.humidityPercent ?? null, '%')}</dd></div><div><dt><Wind />Viento</dt><dd>{selected ? `${selected.windDirection ?? '—'} · ${fmt(selected.windKmh, ' km/h')}` : '—'}</dd></div><div><dt><CloudRain />Precipitación</dt><dd>{fmt(selected?.precipitationProbabilityPercent ?? null, '%')}</dd></div></dl></section>
...
export function WeatherAlertDetailPage() {
  ...
  const isVisualRadar = Boolean(radar?.source.attribution.includes('ilustrativos'));
  const radarImage = isVisualRadar ? '/photos/sierra-magina-radar-visual.png' : latestRadar?.imageUrl;
  return <WeatherShell><WeatherIntro title="Detalle de alerta" copy="Información sobre un aviso meteorológico que puede afectar a tu olivar." />
    ...
    {radarImage ? <figure className="weather-alert-radar"><img src={radarImage} alt={isVisualRadar ? 'Mapa visual de precipitación sobre Sierra Mágina' : 'Último fotograma disponible del radar de precipitación'} /><figcaption><strong>{isVisualRadar ? 'Radar visual de precipitación' : 'Radar de precipitación'}</strong><span>{isVisualRadar ? 'Mapa ilustrativo para la vista demo' : radar?.source.attribution}</span></figcaption></figure> : null}
